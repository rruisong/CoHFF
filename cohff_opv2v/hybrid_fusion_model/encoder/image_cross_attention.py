...
class ImageCrossAttention(BaseModule):
    """An attention module used in pFormer.
    Args:
    ...
    def forward(self,
        ...
        queries = torch.split(query, [self.p_h*self.p_w, self.p_z*self.p_h, self.p_w*self.p_z], dim=1)
        if residual is None:
        ...
        for p_idx, p_mask in enumerate(p_masks):
            indexes = []
            for _, mask_per_img in enumerate(p_mask):
            ...
            reference_points_cam = reference_points_cams[p_idx]
            D = reference_points_cam.size(3)

            queries_rebatch = queries[p_idx].new_zeros(
            ...
            for i, reference_points_per_img in enumerate(reference_points_cam):
                for j in range(bs):
                    ...
                    queries_rebatch[j * self.num_cams + i, :len(index_query_per_img)] = queries[p_idx][j, index_query_per_img]
        ...
        for p_idx, indexes in enumerate(indexeses):
            for i, index_query_per_img in enumerate(indexes):
                for j in range(bs):
                    slots[p_idx][j, index_query_per_img] += queries[p_idx][j * self.num_cams + i, :len(index_query_per_img)]

            count = p_masks[p_idx].sum(-1) > 0
            count = count.permute(1, 2, 0).sum(-1)
            count = torch.clamp(count, min=1.0)
            slots[p_idx] = slots[p_idx] / count[..., None]
...
@ATTENTION.register_module()
class CoHFF_MSDeformableAttention3D(BaseModule):
    """An attention module used in pFormer based on Deformable-Detr.
