...
class CoHFF_Decoder_CrossView_Self_Attention(BaseModule):
    ...
    def __init__(self, 
        ...
        **kwargs
        ...
        if cav_aggregator is True:
            self.dropout = nn.ModuleList([
                nn.Dropout(dropout) for _ in range(5)
            ])
            self.output_proj = nn.ModuleList([
                nn.Linear(embed_dims, embed_dims) for _ in range(5)
            ])
        else:
            self.dropout = nn.ModuleList([
        ...
        self.sampling_offsets = nn.ModuleList([
            nn.Linear(embed_dims, num_heads * 3 * num_points * 2) for _ in range(5)
        ])
        self.attention_weights = nn.ModuleList([
            nn.Linear(embed_dims, num_heads * 3 * (num_points + 1)) for _ in range(5)
        ])
    ...
    def get_sampling_offsets_and_attention(self, queries):
        ...
        for i, query in enumerate(queries):
            # Weights of FC layers are shared 
            if i == 0:
                j = 0
            elif i == 1:
                j = 1
                ...
                j = 4 
            fc = self.sampling_offsets[j]
            attn = self.attention_weights[j]
            bs, l, d = query.shape
    ...
    def reshape_output(self, output, lens):
        ...
        outputs = torch.split(output, lens, dim=1)
        return outputs
    ...
    def forward(self,                
        ...
        value = []
        for i_ele, query_ele in enumerate(query):
            if i_ele == 0:
                layer = self.value_proj[0]
            elif i_ele ==1:
                layer = self.value_proj[1]
            elif i_ele == 2:
                layer = self.value_proj[2]
            elif i_ele % 2 == 1:
                layer = self.value_proj[3]
            else:
                layer = self.value_proj[4]
            value.append(layer(query_ele))
        ...
        if reference_points.shape[-1] == 2:
            """
            For each p query, it owns `num_Z_anchors` in 3D space that having different heights.
            After proejcting, each p query has `num_Z_anchors` reference points in each 2D image.
        ...
        results = []
        for i_ele, (output_ele, residual) in enumerate(zip(outputs, identity)):
            if i_ele == 0:
                layer = self.output_proj[0]
                drop = self.dropout[0]
            elif i_ele ==1:
                layer = self.output_proj[1]
                drop = self.dropout[1]
            elif i_ele == 2:
                layer = self.output_proj[2]
                drop = self.dropout[2]
            elif i_ele % 2 == 1:
                layer = self.output_proj[3]
                drop = self.dropout[3]
            else:
                layer = self.output_proj[4]
                drop = self.dropout[4]
            results.append(residual + drop(layer(output_ele)))
