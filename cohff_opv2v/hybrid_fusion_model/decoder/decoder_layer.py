...
class CoHFF_Decoder_Layer(BaseModule):
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            if layer == 'self_attn':
                ...
                lsi = torch.tensor([
                    0, p_h*p_w, (p_h*p_w+p_z*p_h)
                ], device=query[0].device)


                if not isinstance(query, (list, tuple)):
                    query = torch.split(
                        query, [p_h*p_w, p_z*p_h, p_w*p_z], dim=1)
        ...
        split_size=[p_h*p_w, p_z*p_h, p_w*p_z]
        for _ in range(num_cav):
            split_size.append(p_z*p_h)
            split_size.append(p_w*p_z)

        query = torch.split(query, split_size,dim=1)
        return query
